...
class LockScreenViewController: NSViewController {
    ...
    private func checkPassword() {
        ...
        // Check password
        if settings.checkPassword(enteredPassword) {
            // Success
            ...
            if settings.isLockedOut() {
                ...
                let originalMessage = messageLabel.stringValue
                messageLabel.stringValue = "Incorrect password. \(settings.remainingAttempts()) attempts remaining."
                messageLabel.textColor = .red
                
                // Shake animation
                let animation = CABasicAnimation(keyPath: "position.x")
                animation.fromValue = passwordField.frame.origin.x
                animation.toValue = passwordField.frame.origin.x + 10
                ...
                DispatchQueue.main.asyncAfter(deadline: .now() + 3.0) {
                    self.messageLabel.stringValue = originalMessage
    ...
    private func attemptSystemAuthentication() {
        guard UserSettings.shared.enablePasswordProtection else {
            // If password protection is disabled, just unlock
            delegate?.didUnlockScreen()
            return
        }
            ...
            context.evaluatePolicy(.deviceOwnerAuthenticationWithBiometrics, localizedReason: reason) { [weak self] success, authenticationError in
                DispatchQueue.main.async {
                    if success {
                        ...
                        // Show retry button if biometric auth failed
